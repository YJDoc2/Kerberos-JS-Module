...
export default class KerberosTGS {
  ...
  decryptReq(encReqStr, cUid1, cUid2, tgt) {
    const key = this.verifyTGTandGetKey(cUid1, cUid2, tgt);

    const reqStr = this.cryptor.decrypt(key, encReqStr, TGT_INIT_VAL);
    ...
    try {
      ...
      if (e instanceof SyntaxError) {
        throw new ServerError('Not a Valid request');
      } else {
        throw e;
      }
    }
  ...
  // random as a property, thus an extra method call is reuired.
  verifyRand(cUid1, cUid2, rand) {
    ...
    if (userData) console.log(userData.includes(rand));
    if (!userData) {
   ...
   * @param {Number} lifetimeMs lifetime for generated ticket
   *
